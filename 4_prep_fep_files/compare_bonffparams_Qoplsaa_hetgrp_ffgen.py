...
for line in topol_prm.readlines():
	...
	if counter==1:
		end_prm.seek(0)
		line_elements=[]
		line_elements.append(re.findall('[A-Z-0-9]+', line)[0])
		line_elements+=(re.findall('[0-9]+.[0-9]+', line)[0:6])
		if len(line_elements)>1:
			line_elements=re.findall('[A-Z0-9]+', line)[0:2]+re.findall('[0-9]+\.[0-9]+', line)
			strike=0
			lig1_lib.seek(0)
			atom_data1={}
			atom_nb=0
			for linelib1 in lig1_lib.readlines():
				if ("[bonds]" in linelib1) or (strike==1 and linelib1=="\n"):
					strike=0
					break
				elif "[atoms]" in linelib1:
					strike=1
				elif strike==1:
					atom_nb+=1
		...
		for line2 in end_prm.readlines():
			if str(line2)=="\n" or "!  X" in str(line2):
				...
				line2_elements=[]
				line2_elements.append(re.findall('[A-Z-0-9]+', line2)[0])
				line2_elements+=(re.findall('[0-9]+.[0-9]+', line2)[0:6])
				if len(line2_elements)>1:
					isfile=1
					line2_elements=re.findall('[A-Z0-9]+', line2)[0:2]+re.findall('[0-9]+\.[0-9]+', line2)
					lig2_lib.seek(0)
					strike2=0
					for linelib2 in lig2_lib.readlines():
						if ("[bonds]" in linelib2) or (strike2==1 and linelib2=="\n"):
							strike2=0
							break
						elif "[atoms]" in linelib2:
							strike2=1
						elif strike2==1:
	...
	if counter==2:
		end_prm.seek(0)
		line_elements=[]
		line_elements.append(re.findall('[A-Z-0-9]+', line)[0:3])
		line_elements+=(re.findall('[0-9]+.[0-9]+', line)[0:2])
		if len(line_elements)>1:
			atom_data1={}
			line_elements=re.findall('[A-Z0-9]+', line)[0:3]+re.findall('[0-9]+\.[0-9]+', line)
			lig1_lib.seek(0)
			strike1=0
			atom_nb=0
			for linelib1 in lig1_lib.readlines():
				if ("[bonds]" in linelib1) or (strike1==1 and linelib1=="\n"):
					strike1=0
					break
				elif "[atoms]" in linelib1:
					strike1=1
		...
		for line2 in end_prm.readlines():
			if str(line2)=="\n" or "!  X" in str(line2):
				...
				line2_elements=[]
				line2_elements.append(re.findall('[A-Z-0-9]+', line2)[0:3])
				line2_elements+=(re.findall('[0-9]+.[0-9]+', line2)[0:2])
				if len(line2_elements)>1:
					line2_elements=re.findall('[A-Z0-9]+', line2)[0:3]+re.findall('[0-9]+\.[0-9]+', line2)
					lig2_lib.seek(0)
					strike2=0
					for linelib2 in lig2_lib.readlines():
						if ("[bonds]" in linelib2) or (strike2==1 and linelib2=="\n"):
	...
	if counter==3:
		end_prm.seek(0)
		line_elements=[]
		line_elements.append(re.findall('[A-Z-0-9]+', line)[0:4])
		line_elements+=(re.findall('\D[0-9]\D[0-9]+', line)[0:3])
		if len(line_elements)>1:
			atom_data1={}
			line_elements=re.findall('[A-Z0-9]+', line)[0:4]+re.findall(r"[-+]?\d*\.\d+|\d+", line)[4:7]
			for i in range(0,4):
				if line_elements[i] == "TH" and "H_" in line:
					line_elements[i]="TH_"
			lig1_lib.seek(0)
			strike1=0
			atom_nb=0
			...
			if atom1+' '+atom1b+' '+atom1c+' '+atom1d not in term_data1 and len(atom_data1)==4:
				redundancy=0
				term1_count=1
			...
			for line2 in end_prm.readlines():
				if str(line2)=="\n" or "!  X" in str(line2):
					...
					line2_elements=[]
					line2_elements.append(re.findall('[A-Z-0-9]+', line2)[0:4])
					line2_elements+=(re.findall('\D[0-9]\D[0-9]+', line2)[0:3])
					if len(line2_elements)>1:
						line2_elements=re.findall('[A-Z0-9]+', line2)[0:4]+re.findall(r"[-+]?\d*\.\d+|\d+", line2)[4:7]
						for i in range(0,4):
							if line2_elements[i] == "TH" and "H_" in line2:
								line2_elements[i]="TH_"
						lig2_lib.seek(0)
						strike2=0
						for linelib2 in lig2_lib.readlines():
							if ("[bonds]" in linelib2) or (strike2==1 and linelib2=="\n"):
		...
		for line2 in end_prm.readlines():
			if str(line2)=="\n" or "!  X" in str(line2):
				...
				if len(line2_elements)>1:
					line2_elements=re.findall('[A-Z0-9]+', line2)[0:4]+re.findall('[0-9]+\.[0-9]+', line2)[0:3]
					lig2_lib.seek(0)
					strike2=0
