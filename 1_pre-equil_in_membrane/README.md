...
- Remove the nanobody if any (typically "resi 1000-*"), as well as ligands and crystallization agents ("organic"), waters ("resn HOH") and ions ("metal"). PS. Removing the ligand will allow for waters come solvate the receptor properly at that stage.
...
```modX.XX add_missing_atoms_modeller.py YOUR_SAVED_STRUCTURE.pdb``` # X.XX according to your modeller version. The output is named like YOUR_SAVED_STRUCTURE_full.pdb. 
...
- Also, load the water model you want to use (.itp file), if not SPC, at the beginning of the topol.top file.
